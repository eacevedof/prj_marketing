...
class HelperTableBasic extends HelperTable
{
    ...
    protected function build_js()
    {
        ...
        if($this->arAssignMulti) {
            $sHtmlJs .= $this->js_fn_multiassign_window();
        }
        if($this->arMultiAdd) {
            $sHtmlJs .= $this->js_fn_multiadd();
        }
        if($this->arAssignSingle) {
            $sHtmlJs .= $this->js_fn_singleassign_window();
        }
        if($this->arSingleAdd) {
            $sHtmlJs .= $this->js_fn_singleadd();
        }
        $sHtmlJs .= "</script>\n";
        return $sHtmlJs;
    }
    ...
    protected function get_operation_columns()
    {
        $arColumns = [];
        if($this->isToDetail) {
            $arColumns["detail"] = "Upd";
        }
        if($this->isPickMultiple) {
            $arColumns["multipick"] = "Multi";
        }
        if($this->isPickSingle) {
            $arColumns["singlepick"] = "Single";
        }

        if(!empty($arColumns)) {
            $arColumns = array_merge($arColumns, $this->arColumns);
        } else {
            $arColumns = $this->arColumns;
        }

        if($this->isDeleteSingle) {
            $arColumns["delete"] = "Del";
        }
        if($this->isQuarantineSingle) {
            $arColumns["quarantine"] = "Del";
        }
        //bug($arColumns,"operation_columns");
        if($this->arExtraColumns) {
            $arColumns = $this->reorder_columns($arColumns, $this->arExtraColumns);
        }
        return $arColumns;
    }
    ...
    protected function build_column_label($sFieldName, $sLabel)
    {
        ...
        if($this->isOrdenable && $this->arOrderBy) {
            ...
            if($iPosition || $iPosition === 0) {
                $sOrderWay = $this->arOrderWay[$iPosition];
            }
    ...
    protected function build_navigation_buttons($sHtmlSelect)
    {
        ...
        if($this->iInfoNumPages > 1) {
            $sHtmlUlButtons .= "<li>&nbsp;&nbsp;Go to:&nbsp;&nbsp;$sHtmlSelect</li>";
        }
        $sHtmlUlButtons .= "</ul>";
    ...
    private function build_url_nomethod($isByMvc = 0)
    {
        ...
        if($this->isPermaLink) {
            ...
            if($isByMvc) {
                if(isset($_GET["tfw_package"])) {
                    $arUrlNoMethod[1] = $_GET["tfw_package"];
                }
                if(isset($_GET["tfw_controller"])) {
                    $arUrlNoMethod[2] = $_GET["tfw_controller"];
                }
                if(isset($_GET["tfw_partial"])) {
                    $arUrlNoMethod[3] = $_GET["tfw_partial"];
                }
            }
            //isByMvc=0
            else {
                if(isset($_GET["tfw_group"])) {
                    $arUrlNoMethod[1] = $_GET["tfw_group"];
                }
                if(isset($_GET["tfw_module"])) {
                    $arUrlNoMethod[2] = $_GET["tfw_module"];
                }
                if(isset($_GET["tfw_section"])) {
                    $arUrlNoMethod[3] = $_GET["tfw_section"];
                }
            }
        ...
        //no permalink
            ...
            if($isByMvc) {
                if($_GET["tfw_package"]) {
                    $arUrlNoMethod[1] = "package=".$_GET["tfw_package"];
                }
                if($_GET["tfw_controller"]) {
                    $arUrlNoMethod[2] = "controller=".$_GET["tfw_controller"];
                }
                if($_GET["tfw_partial"]) {
                    $arUrlNoMethod[3] = "partial=".$_GET["tfw_partial"];
                }
            }
            //isByMvc=0
            else {
                if($_GET["tfw_group"]) {
                    $arUrlNoMethod[1] = "group=".$_GET["tfw_group"];
                }
                if($_GET["tfw_module"]) {
                    $arUrlNoMethod[2] = "module=".$_GET["tfw_module"];
                }
                if($_GET["tfw_section"]) {
                    $arUrlNoMethod[3] = "section=".$_GET["tfw_section"];
                }
            }
            return implode("&", $arUrlNoMethod);
        }
    ...
    public function set_column_detail($isOn = true)
    {
        $this->isToDetail = $isOn;
    }
    public function set_column_delete($isOn = true)
    {
        $this->isDeleteSingle = $isOn;
    }
    public function set_column_quarantine($isOn = true)
    {
        $this->isQuarantineSingle = $isOn;
    }
    public function set_column_pickmultiple($isOn = true)
    {
        $this->isPickMultiple = $isOn;
    }
    public function merge_pks($isOn = true, $sGlue = ",")
    {
        $this->doMergePkeys = $isOn;
        $this->sMergeGlue = $sGlue;
    }
    public function set_column_picksingle($isOn = true)
    {
        $this->isPickSingle = $isOn;
    }
    public function set_keyfields($arKeyFields)
    {
        $this->arKeyFields = $arKeyFields;
    }
    public function set_orderby($arFieldNames)
    {
        $this->arOrderBy = $arFieldNames;
    }
    public function set_orderby_type($arOrderWay)
    {
        $this->arOrderWay = $arOrderWay;
    }
    public function set_url_delete($sUrl)
    {
        $this->sUrlDelete = $sUrl;
    }
    public function set_url_quarantine($sUrl)
    {
        $this->sUrlQuarantine = $sUrl;
    }
    public function set_url_update($sUrl)
    {
        $this->sUrlUpdate = $sUrl;
    }
    /**
     ...
     * @param string $sUrl
     */
    public function set_url_paginate($sUrl)
    {
        $this->sUrlPaginate = $sUrl;
    }
    public function set_url_picksingle($sUrl)
    {
        $this->sUrlPickSingle = $sUrl;
    }
    ...
    public function set_current_page($iNumPage)
    {
        $this->iInfoCurrentPage = $iNumPage;
    }
    public function set_next_page($iNumPage)
    {
        $this->iInfoNextPage = $iNumPage;
    }
    public function set_previous_page($iNumPage)
    {
        $this->iInfoPreviousPage = $iNumPage;
    }
    public function set_total_regs($iNumRegs)
    {
        $this->iInfoNumRegs = $iNumRegs;
    }
    public function set_total_pages($iNumPages)
    {
        $this->iInfoNumPages = $iNumPages;
    }
    public function set_first_page($iNumFirstPage)
    {
        $this->iInfoFirstPage = $iNumFirstPage;
    }
    public function set_last_page($iNumLastPage)
    {
        $this->iInfoLastPage = $iNumLastPage;
    }
    public function set_fields($arObjFields)
    {
        $this->arObjFields = $arObjFields;
    }
    public function is_ordenable($isOn = true)
    {
        $this->isOrdenable = $isOn;
    }
    public function set_check_onrowclick($isOn = true)
    {
     ...
     */
    public function set_multiassign($arData)
    {
        $this->arAssignMulti = $arData;
    }
    public function set_singleassign($arData)
    {
        $this->arAssignSingle = $arData;
    }

    public function set_multiadd($arData)
    {
        $this->arMultiAdd = $arData;
    }
    /**
     ...
     */
    public function set_singleadd($arData)
    {
        $this->arSingleAdd = $arData;
    }
    /**
     * format: ("date","datetime4","datetime6","time4","time6","int","numeric2")
     * @param array $arFormat array("fieldname"=>"format"...)
     */
    public function set_format_columns($arFormat)
    {
        $this->arConfigColTypes = $arFormat;
    }

    /**
     * @param array $arData array(0=>array("position"=>1,"label"=>"xxx","type"=>"anchor","href"=>"localhost/","hrefparams"=>array("col1","col2",)))
     */
    public function add_extra_colums($arData)
    {
        $this->arExtraColumns = $arData;
    }

    /**
     * @param array $arColumns array("fieldname1","fieldname2"..)
     */
    public function set_column_hidden($arColumns)
    {
        $this->arHiddenColumns = $arColumns;
    }
     ...
     */
    public function set_extra_hidden($arColumns)
    {
        $this->arExtraHidden = $arColumns;
    }

    public function set_items_per_page($iItemsPerPage)
    {
     ...
     * @param type $isOn
     */
    public function set_no_paginatebar($isOn = false)
    {
        $this->isPaginateBar = $isOn;
    }

    public function set_column_length($arColumnLength)
    {
        $this->arColumnLength = [];
        if($arColumnLength) {
            $this->arColumnLength = $arColumnLength;
        }
    }
    public function add_column_length($sFieldName, $iLength)
    {
        $this->arColumnLength[$sFieldName] = $iLength;
    }
    //**********************************
    //             GETS
    //**********************************
    ...
    protected function has_column_length($sFieldName)
    {
        $arFieldNames = array_keys($this->arColumnLength);
        return in_array($sFieldName, $arFieldNames);
    }
    protected function get_column_length($sFieldName)
    {
        return $this->arColumnLength[$sFieldName];
    }
