...
class HelperDate extends TheFrameworkHelper
{
    ...
    public function __construct($id = "", $name = "", $value = "", $arExtras = array(), $maxlength = "", $class = "", HelperLabel $oLabel = null)
    {
        //$this->_type = "date";
    ...
    public function get_html()
    {
        //$this->isIpadIphone = 1;
        $sHtmlToReturn = "";
        if($this->_isReadOnly || $this->isIpadIphone) {
            $this->_type = "text";
        }
        if($this->oLabel) {
            $sHtmlToReturn .= $this->oLabel->get_html();
        }
        if($this->_comments) {
        ...
        $sHtmlToReturn .= "<input";
        if($this->_type) {
            $sHtmlToReturn .= " type=\"$this->_type\"";
        }
        if($this->_id) {
            $sHtmlToReturn .= " id=\"$this->_idprefix$this->_id\"";
        }
        if($this->_name) {
            $sHtmlToReturn .= " name=\"$this->_idprefix$this->_name\"";
        }
        if($this->_convert_date_before_show) {
            $this->_value = $this->to_user_date($this->_value);
        }
        if($this->_value) {
        ...
        //propiedades html5
        if($this->_maxlength) {
            $sHtmlToReturn .= " maxlength=\"$this->_maxlength\"";
        }
        if($this->_isDisabled) {
            $sHtmlToReturn .= " disabled";
        }
        if($this->_isReadOnly) {
            $sHtmlToReturn .= " readonly";
        }
        if($this->_isRequired) {
            $sHtmlToReturn .= " required";
        }
        //eventos
        if($this->_js_onblur) {
            $sHtmlToReturn .= " onblur=\"$this->_js_onblur\"";
        }
        if($this->_js_onchange && $this->_isPostback) {
            $sHtmlToReturn .= " onchange=\"$this->_js_onchange;postback(this);\"";
        } elseif($this->_js_onchange) {
            $sHtmlToReturn .= " onchange=\"$this->_js_onchange\"";
        }
        //postback(): Funcion definida en HelperJavascript
        elseif($this->_isPostback) {
            $sHtmlToReturn .= " onchange=\"postback(this);\"";
        }

        if($this->_js_onclick) {
            $sHtmlToReturn .= " onclick=\"$this->_js_onclick\"";
        }
        ...
        if($this->_js_onkeypress && $this->_isEnterInsert) {
            ...
            $sHtmlToReturn .= " onkeypress=\"$this->_js_onkeypress\"";
        }
        //postback(): Funcion definida en HelperJavascript
        elseif($this->_isEnterInsert) {
            $sHtmlToReturn .= " onkeypress=\"onenter_insert(event);\"";
        } elseif($this->_isEnterUpdate) {
            $sHtmlToReturn .= " onkeypress=\"onenter_update(event);\"";
        } elseif($this->_isEnterSubmit) {
            $sHtmlToReturn .= " onkeypress=\"onenter_submit(event);\"";
        }

        if($this->_js_onfocus) {
            $sHtmlToReturn .= " onfocus=\"$this->_js_onfocus\"";
        }
        if($this->_js_onmouseover) {
            $sHtmlToReturn .= " onmouseover=\"$this->_js_onmouseover\"";
        }
        if($this->_js_onmouseout) {
            $sHtmlToReturn .= " onmouseout=\"$this->_js_onmouseout\"";
        }

        //aspecto
        $this->load_cssclass();
        if($this->_class) {
            $sHtmlToReturn .= " class=\"$this->_class\"";
        }
        $this->load_style();
        if($this->_style) {
            $sHtmlToReturn .= " style=\"$this->_style\"";
        }

        //atributos extras 18/04/2014 este atributo lo dejo siempre para cualquier terminal
        if(!$this->_isReadOnly) {
            $sHtmlToReturn .= " as=\"date\"";
        }

        if($this->_placeholder) {
            $sHtmlToReturn .= " placeholder=\"$this->_placeholder\"";
        }
        if($this->_isPrimaryKey) {
            $sHtmlToReturn .= " pk=\"pk\"";
        }
        if($this->_attr_dbfield) {
            $sHtmlToReturn .= " dbfield=\"$this->_attr_dbfield\"";
        }
        if($this->_attr_dbtype) {
            $sHtmlToReturn .= " dbtype=\"$this->_attr_dbtype\"";
        }
        if($this->arExtras) {
            $sHtmlToReturn .= " ".$this->get_extras();
        }
    ...
    public function set_value($value, $asEntity = 0)
    {
    ...
    public function set_is_ipadiphone($isOn = true)
    {
        $this->isIpadIphone = $isOn;
    }
    public function set_separator($sValue)
    {
        $this->cSeparator = $sValue;
    }
    ...
    public function get_value($asEntity = 0)
    {
        if($asEntity) {
