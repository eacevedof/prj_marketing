...
class HelperCheckbox extends TheFrameworkHelper
{
    ...
    public function __construct(
        $mxOptions = array(),
        $name = "",
        $mxValuesToCheck = array(),
        $mxValuesDisabled = array(),
        $class = "",
        $arExtras = "",
        $isGrouped = true,
        HelperLegend $oLegend = null,
        HelperFieldset $oFieldset = null
    ) {
    ...
    public function get_html()
    {
        ...
        if($this->oLegend) {
            $sHtmlToReturn .= $this->oLegend->get_html();
        }
        ...
        foreach($this->arOptions as $sValue => $sOutText) {
            ...
            if($this->_id) {
                $sCheckId = "$this->_idprefix$this->_id";
            }
            if($iNumOptions > 1) {
                $sCheckId .= "_$iOption";
            }
            ...
            if(($iOption % ($this->iChecksPerLine)) == 0 && $iOption > 0) {
                $sHtmlToReturn .= "<br/>";
            }
    ...
    private function build_check($id, $sValue, $sOutText, $isChecked = false, $isReadOnly = false)
    {
        ...
        if($this->_js_onkeypress && $this->_isEnterInsert) {
            ...
            $sHtmlCheckbox .= " onkeypress=\"$this->_js_onkeypress\"";
        }
        //postback(): Funcion definida en HelperJavascript
        elseif($this->_isEnterInsert) {
            $sHtmlCheckbox .= " onkeypress=\"onenter_insert(event);\"";
        } elseif($this->_isEnterUpdate) {
            $sHtmlCheckbox .= " onkeypress=\"onenter_update(event);\"";
        } elseif($this->_isEnterSubmit) {
            $sHtmlCheckbox .= " onkeypress=\"onenter_submit(event);\"";
        }

        if($this->_js_onfocus) {
            $sHtmlCheckbox .= " onfocus=\"$this->_js_onfocus\"";
        }
        if($this->_js_onmouseover) {
            $sHtmlCheckbox .= " onmouseover=\"$this->_js_onmouseover\"";
        }
        if($this->_js_onmouseout) {
            $sHtmlCheckbox .= " onmouseout=\"$this->_js_onmouseout\"";
        }

        //aspecto
        $this->load_cssclass();
        if($this->_class) {
            $sHtmlCheckbox .= " class=\"$this->_class\"";
        }
        $this->load_style();
        if($this->_style) {
            $sHtmlCheckbox .= " style=\"$this->_style\"";
        }
        //atributos extras
        if($this->_attr_dbfield) {
            $sHtmlCheckbox .= " dbfield=\"$this->_attr_dbfield\"";
        }
        if($this->_attr_dbtype) {
            $sHtmlCheckbox .= " dbtype=\"$this->_attr_dbtype\"";
        }
        if($this->arExtras) {
            $sHtmlCheckbox .= " ".$this->get_extras();
        }

        if($isChecked) {
            $sHtmlCheckbox .= " checked";
        }
        if($isReadOnly) {
            $sHtmlCheckbox .= " disabled";
        }
        $sHtmlCheckbox .= ">";
        //out text
        if($this->isLabeled) {
    ...
    public function set_values_to_check($mxValues)
    {
        $this->conv_string_to_array($mxValues);
        $this->arValuesToCheck = $mxValues;
    }
    ...
    {
        $this->iChecksPerLine = $iNumChecks;
    }
    public function set_options($mxOptions)
    {
        $this->conv_string_to_array($mxOptions, 1);
        $this->arOptions = $mxOptions;
    }
    public function set_unlabeled($isOn = true)
    {
        $this->isLabeled = !$isOn;
    }
