...
class HelperTableTyped extends HelperTableBasic
{
    ...
    protected function get_operation_columns()
    {
        ...
        if($this->isColumnButtonUpdate) {
            $arColumns["butupdate"] = "Save";
        }
        if($this->isColumnButtonInsert) {
            $arColumns["butinsert"] = "New";
        }
        return $arColumns;
    }
    ...
    protected function build_anchor_cell_content($arRow, $sFieldName, $iNumRow, $iNumColumn)
    {
        ...
        if($sHref !== "%nohref%") {
            ...
            //@TODOTEMPLATE
            if($sClassIcon) {
                $sInnerHtml = "<span class=\"$sClassIcon\"></span> $sInnerHtml";
            }
    ...
    protected function build_inputtext_cell_content($arRow, $sFieldName, $iNumRow, $iNumColumn)
    {
        ...
        //bug($arProperties);
        if($arProperties["class"]) {
            $oInputText->add_class($arProperties["class"]);
        }
        //@TODOTEMPLATE
        else {
            $oInputText->add_class("input-small");
        }
        if($arProperties["onclick"]) {
            $oInputText->set_js_onclick($arProperties["onclick"]);
        }
        if($arProperties["onfocus"]) {
            $oInputText->set_js_onfocus($arProperties["onfocus"]);
        }
        if($arProperties["readonly"]) {
            $oInputText->readonly();
        }
    ...
    protected function get_anchor_data($arRow, $sFieldName)
    {
        ...
        if(!$arAnchorData["href"]) {
            $arAnchorData["href"] = $arConfigData["href"];
        }
        ...
        if(!$arAnchorData["innerhtml"]) {
            $arAnchorData["innerhtml"] = $arConfigData["innerhtml"];
        }

        if($arConfigData["external"]) {
            $arAnchorData["external"] = $arConfigData["external"];
        }
        if($arConfigData["target"]) {
            $arAnchorData["target"] = $arConfigData["target"];
        }

        if($arConfigData["class"]) {
            $arAnchorData["class"] = $arConfigData["class"];
        }

        if($arConfigData["icon"]) {
            $arAnchorData["icon"] = $arConfigData["icon"];
        }
        return $arAnchorData;
    }//fin get_anchor_data
    ...
    protected function get_select_options($sFieldName)
    {
        return $this->arColumnsSelect[$sFieldName];
    }
    //**********************************
    //             SETS
    //**********************************
     ...
     */
    public function set_column_anchor($arColumns)
    {
        $this->arColumnsAnchor = $arColumns;
    }
    public function set_column_text($arColumns)
    {
        $this->arColumnsInputText = $arColumns;
    }
    public function set_column_select($arColumns)
    {
        $this->arColumnsSelect = $arColumns;
    }
    public function set_column_checkbox($arColumns)
    {
        $this->arColumnsCheckbox = $arColumns;
    }
    public function set_insert_button($isOn = true)
    {
        $this->isColumnButtonInsert = $isOn;
    }
    public function set_update_button($isOn = true)
    {
        $this->isColumnButtonUpdate = $isOn;
    }
    public function set_column_raw($arColumns)
    {
        $this->arColumnsRaw = $arColumns;
    }
    //**********************************
    //             GETS
    //**********************************
