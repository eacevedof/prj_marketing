...
class HelperTextarea extends TheFrameworkHelper
{
    ...
    public function __construct($id = "", $name = "", $innerhtml = "", $arExtras = "", $maxlength = -1, $cols = 40, $rows = 8, $class = "", $style = "", HelperLabel $oLabel = null)
    {
        $this->_type = "textarea";
...
<?php
    ...
    public function get_opentag()
    {
        ...
        if($this->_name) {
            $sHtmlOpenTag .= "name=\"$this->_idprefix$this->_name\" ";
        }
        if($this->_rows) {
            $sHtmlOpenTag .= "rows=\"$this->_rows\" ";
        }
        if($this->_cols) {
            $sHtmlOpenTag .= "cols=\"$this->_cols\" ";
        }
        //propiedades html5
        if($this->_isDisabled) {
            $sHtmlOpenTag .= "disabled ";
        }
        if($this->_isReadOnly) {
            $sHtmlOpenTag .= "readonly ";
        }
        if($this->_isRequired) {
            $sHtmlOpenTag .= "required ";
        }
        //eventos
        if($this->_js_onfocus) {
        ...
        if($this->_js_onkeydown) {
            $sHtmlOpenTag .= "onkeydown=\"$this->_js_onkeydown\" ";
        }
        if($this->_js_onkeyup) {
            $sHtmlOpenTag .= "onkeyup=\"$this->_js_onkeyup\" ";
        }
        ...
        //atributos extras
        if($this->_maxlength) {
            $sHtmlOpenTag .= "maxlength=\"$this->_maxlength\" ";
        }
        if($this->arExtras) {
    ...
    public function set_rows($iValue)
    {
        $this->_rows = $iValue;
    }
    public function set_cols($iValue)
    {
        $this->_cols = $iValue;
    }
    public function set_counterspan($isOn = true)
    {
        $this->isCounterSpan = $isOn;
    }
    public function set_counterjs($isOn = true)
    {
