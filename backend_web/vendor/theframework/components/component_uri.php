...
class ComponentUri //extends TheFrameworkComponent
{
    ...
    public function get_built()
    {
        ...
        if ($this->isPermaLink) {
            ...
            if ($this->sIsoLanguage) {
                $arUrl[] = $this->sIsoLanguage;
            }
            if ($this->sPackage) {
                $arUrl[] = $this->sPackage;
            }
            if ($this->sController) {
                $arUrl[] = $this->sController;
            }
            if ($this->sPartial) {
                $arUrl[] = $this->sPartial;
            }
        ...
        //& params url
            ...
            $cGlue = "&";
            if ($this->sIsoLanguage) {
                $arUrl[] = "lang=$this->sIsoLanguage";
            }
            if ($this->sPackage) {
                $arUrl[] = "group=$this->sPackage";
            }
            if ($this->sController) {
                $arUrl[] = "module=$this->sController";
            }
            if ($this->sPartial) {
                $arUrl[] = "section=$this->sPartial";
            }
            $sMethod = $this->sMethod;
            if (!$sMethod) {
                $sMethod = "get_list";
    ...
    public function set_iso_language($sValue)
    {
        $this->sIsoLanguage = $sValue;
    }
    public function set_permalink($isOn = true)
    {
        $this->isPermaLink = $isOn;
    }
    public function set_package($sValue)
    {
        $this->sPackage = $sValue;
    }
    public function set_controller($sValue)
    {
        $this->sController = $sValue;
    }
    public function set_partial($sValue)
    {
        $this->sPartial = $sValue;
    }
    public function set_method($sValue)
    {
        $this->sMethod = $sValue;
    }
    public function set_extra($mxValue)
    {
        $this->load_extra($mxValue);
    }
    public function add_extra($sKey, $sValue)
    {
        $this->arExtra[] = "$sKey=$sValue";
    }
    public function add_extra1($sKeyValue)
    {
        $this->arExtra[] = $sKeyValue;
    }
}//ComponentUri
