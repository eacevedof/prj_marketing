...
final class ComponentQB
{
    ...
    private function _get_having(): string
    {
        ...
        $arsql = [];
        foreach($this->arhaving as $havecond) {
            $arsql[] = $havecond;
        }
    ...
    private function _get_limit(): string
    {
        ...
        if ($this->arlimit["regfrom"] === 0) {
            return " LIMIT {$this->arlimit["perpage"]}";
        }
    ...
    private function _is_numeric(string $fieldname): bool
    {
        return in_array($fieldname, $this->arnumeric);
    }

    private function _is_reserved(string $word): bool
    {
        return in_array(strtolower($word), $this->reserved);
    }
    ...
    public function insert(?string $table = null, ?array $arfieldval = null): self
    {
        ...
        if (!$table) {
            $this->_exception("missing table in insert");
        }

        $comment = $this->comment ? "/*$this->comment*/" : "/*insert*/";
        $arfieldval = $arfieldval ?? $this->arinsertfv;
        if (!$arfieldval) {
            $this->_exception("missing fields and values in insert");
        }
    ...
    public function update(?string $table = null, ?array $arfieldval = null, ?array $arpks = null): self
    {
        ...
        if (!$table) {
            $this->_exception("missing table in update");
        }

        $comment = $this->comment ? "/*$this->comment*/" : "/*update*/";
        $arfieldval = $arfieldval ?? $this->arupdatefv;
        if (!$arfieldval) {
            $this->_exception("missing fields and values in update");
        }
    ...
    public function delete(?string $table = null, ?array $arpks = null): self
    {
        ...
        if (!$table) {
            $this->_exception("missing table in delete");
        }
    ...
    public function select(?string $table = null, ?array $fields = null, ?array $arpks = null): self
    {
        ...
        if (!$table) {
            $this->_exception("missing table in select");
        }

        $fields = $fields ?? $this->argetfields;
        if (!$fields) {
            $this->_exception("missing fields in select");
        }
    ...
    public function set_insert_fv(array $arfieldval = []): self
    {
        $this->arinsertfv = [];
        if (is_array($arfieldval)) {
            $this->arinsertfv = $arfieldval;
        } return $this;
    }
    public function add_insert_fv(string $fieldname, $strval, bool $dosanit = true): self
    {
        $this->arinsertfv[$fieldname] = ($dosanit) ? $this->getSanitizedString($strval) : $strval;
        return $this;
    }

    public function set_pks_fv(array $arfieldval = []): self
    {
        $this->arpks = [];
        if (is_array($arfieldval)) {
            $this->arpks = $arfieldval;
        } return $this;
    }
    public function add_pk_fv(string $fieldname, $strval, bool $dosanit = true): self
    {
        $this->arpks[$fieldname] = ($dosanit) ? $this->getSanitizedString($strval) : $strval;
        return $this;
    }

    public function set_update_fv(array $arfieldval = []): self
    {
        $this->arupdatefv = [];
        if (is_array($arfieldval)) {
            $this->arupdatefv = $arfieldval;
        } return $this;
    }
    public function add_update_fv(string $fieldname, $strval, bool $dosanit = true): self
    {
        $this->arupdatefv[$fieldname] = ($dosanit) ? $this->getSanitizedString($strval) : $strval;
        return $this;
    }

    public function set_getfields(array $fields = []): self
    {
        $this->argetfields = [];
        if (is_array($fields)) {
            $this->argetfields = $fields;
        } return $this;
    }
    public function add_getfield(string $fieldname): self
    {
        $this->argetfields[] = $fieldname;
        return $this;
    }

    public function set_joins(array $arjoins = []): self
    {
        $this->arjoins = [];
        if (is_array($arjoins)) {
    ...
    public function set_orderby(array $arorderby = []): self
    {
        $this->arorderby = [];
    ...
    public function set_groupby(array $argroupby = []): self
    {
        $this->argroupby = [];
    ...
    public function set_having(array $arhaving = []): self
    {
        $this->arhaving = [];
        if (is_array($arhaving)) {
            $this->arhaving = $arhaving;
        } return $this;
    }

    public function set_end(array $arend = []): self
    {
        $this->arend = [];
        if (is_array($arend)) {
            $this->arend = $arend;
    ...
    public function getSanitizedString(?string $strval): ?string
    {
        if ($strval === null) {
            return null;
        }
    ...
    public function exec(string $mode = self::READ)
    {
        $result = [];
        if (!$this->oDB) {
            $this->_exception("no db object not configured for get_result");
        }
        if (!$this->sql) {
            $this->_exception("empty sql in get_result");
        }
        if (!in_array($mode, [self::READ, self::WRITE])) {
            $this->_exception("unrecognized mode");
        }
    ...
    public function distinct(bool $ison = true): self
    {
        $this->isdistinct = $ison;
        return $this;
    }
    public function calcfoundrows(bool $ison = true): self
    {
        $this->calcfoundrows = $ison;
        return $this;
    }
    public function add_numeric(string $fieldname): self
    {
        $this->arnumeric[] = $fieldname;
        return $this;
    }
    public function set_and(array $arands = []): self
    {
        $this->arands = [];
        if (is_array($arands)) {
            $this->arands = $arands;
    ...
    public function add_and(string $condition): self
    {
        $this->arands[] = $condition;
        return $this;
    }
    public function add_andoper(string $fieldname, $strval, string $sOper = "="): self
    {
        $this->arands[] = "$fieldname $sOper $strval";
        return $this;
    }
    ...
    public function add_join(string $sjoin, ?string $key = null): self
    {
        if ($key) {
            $this->arjoins[$key] = $sjoin;
        } else {
            $this->arjoins[] = $sjoin;
        } return $this;
    }
    public function add_orderby(string $fieldname, string $order = "ASC"): self
    {
        $this->arorderby[$fieldname] = $order;
        return $this;
    }
    public function add_groupby(string $fieldname): self
    {
        $this->argroupby[] = $fieldname;
        return $this;
    }
    public function add_having(string $having): self
    {
        $this->arhaving[] = $having;
        return $this;
    }
    public function add_end(string $strend, ?string $key = null): self
    {
        if ($key) {
            $this->arend[$key] = $strend;
        } else {
            $this->arend[] = $strend;
        } return $this;
    }
