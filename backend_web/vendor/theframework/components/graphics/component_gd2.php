...
class ComponentGd2
{
    ...
    private function get_type($sFilename)
    {
        return end(explode(".", trim($sFilename)));
    }

    private function get_image_obj($sExtension, $sPathFile)
    {
        switch($sExtension) {
            case "png": $oImage = imagecreatefrompng($sPathFile);
                break;
            case "gif": $oImage = imagecreatefromgif($sPathFile);
                break;
            case "bmp": $oImage = imagecreatefromwbmp($sPathFile);
                break;
            default:
    ...
    private function save_in_blank($arFrom, $arTo)
    {
        ...
        return imagecopyresampled($arTo["object"], $arFrom["object"], $arTo["x"], $arTo["y"], $arFrom["x"], $arFrom["y"], $arTo["w"], $arTo["h"], $arFrom["w"], $arFrom["h"]);
    }//save_in_blank
    ...
    public function resize($arTo)
    {
        ...
        if (!$sPathFileFrom) {
            $this->add_error("Ruta de origen no proporcionada");
        }
        if (!$this->arTo["pathfile"]) {
            $this->add_error("Ruta de destino no proporcionada");
        }
        if (!is_file($sPathFileFrom)) {
            $this->add_error("Archivo no encontrado en $sPathFileFrom");
        }
    ...
    public function add_from($sKey, $sValue)
    {
        $this->arFrom[$sKey] = $sValue;
    }
    public function add_to($sKey, $sValue)
    {
        $this->arTo[$sKey] = $sValue;
    }
    ...
    {
        echo "<pre>".var_export($this->arErrors, 1);
    }
}//class ComponentGd2
