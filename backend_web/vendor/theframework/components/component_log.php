...
class ComponentLog
{
    ...
    public function __construct($sSubfType = "", $sPathFolder = "")
    {
        ...
        if (!$sPathFolder) {
            $this->sPathFolder = __DIR__;
        }
        if (!$sSubfType) {
            $this->sSubfType = "debug";
        }
    ...
    public function set_filename($sValue)
    {
        $this->sFileName = "$sValue.log";
    }
    public function set_subfolder($sValue)
    {
        $this->sSubfType = "$sValue";
    }
