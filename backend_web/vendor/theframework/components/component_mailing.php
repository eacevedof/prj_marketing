...
class ComponentMailing
{
    ...
    public function __construct($mxEmailTo = "", $sSubject = "", $mxContent = "")
    {
        parent::__construct();
        $this->sFromTitle = "";
        if (defined("APP_MAIL_ISSMTP") && APP_MAIL_ISSMTP !== "") {
            $this->isSmtpUse = APP_MAIL_ISSMTP;
        }
        if (defined("APP_MAIL_SMTP_HOST") && APP_MAIL_SMTP_HOST !== "") {
            $this->sSmtpHost = APP_MAIL_SMTP_HOST;
        }
        if (defined("APP_MAIL_SMTP_PORT") && APP_MAIL_SMTP_PORT !== "") {
            $this->sSmtpPort = APP_MAIL_SMTP_PORT;
        }
        if (defined("APP_MAIL_SMTP_AUTH") && APP_MAIL_SMTP_AUTH !== "") {
            $this->isSmtpAuth = APP_MAIL_SMTP_AUTH;
        }
        if (defined("APP_MAIL_SMTP_USER") && APP_MAIL_SMTP_USER !== "") {
            $this->sSmtpUser = APP_MAIL_SMTP_USER;
        }
        if (defined("APP_MAIL_SMTP_PASSW")) {
            $this->sSmtpPassw = APP_MAIL_SMTP_PASSW;
        }
        if ($this->sSmtpUser) {
            $this->sEmailFrom = $this->sSmtpUser;
        }
    ...
    private function send_pear()
    {
        ...
        try {
            ...
            if ($this->arEmailsCc) {
                $arHeaders["Cc"] = implode(", ", $this->arEmailsCc);
            }
            if ($this->arEmailsBcc) {
                $sBcc = ", ".implode(", ", $this->arEmailsBcc);
            }
    ...
    public function set_email_from($sEmail)
    {
        $this->sEmailFrom = $sEmail;
    }
    public function set_emails_to($mxEmails)
    {
        $this->mxEmailsTo = $mxEmails;
    }
    public function add_email_to($sEmail)
    {
        $this->mxEmailsTo[] = $sEmail;
    }
    public function set_emails_cc($arEmails)
    {
        $this->arEmailsCc = $arEmails;
    }
    public function add_email_cc($sEmail)
    {
        $this->arEmailsCc[] = $sEmail;
    }
    public function set_emails_bcc($arEmails)
    {
        $this->arEmailsBcc = $arEmails;
    }
    public function add_email_bcc($sEmail)
    {
        $this->arEmailsBcc[] = $sEmail;
    }
    public function set_header($sHeader)
    {
        $this->sHeader = $sHeader;
    }
    public function set_content($mxContent)
    {
        (is_array($mxContent)) ? $this->sContent = implode(PHP_EOL, $mxContent) : $this->sContent = $mxContent;
    }
    public function set_title_from($sTitle)
    {
        $this->sFromTitle = $sTitle;
    }
     ...
     */
    public function add_header($sHeader)
    {
        $this->arHeaders[] = $sHeader;
    }
    ...
    public function set_smtp_use($isOn = true)
    {
        $this->isSmtpUse = $isOn;
    }
    public function set_smtp_host($sValue)
    {
        $this->sSmtpHost = $sValue;
    }
    public function set_smtp_port($sValue)
    {
        $this->sSmtpPort = $sValue;
    }
    public function set_smtp_auth($isOn = true)
    {
        $this->isSmtpAuth = $isOn;
    }
    public function set_smtp_user($sValue)
    {
        $this->sSmtpUser = $sValue;
    }
    public function set_smtp_passw($sValue)
    {
        $this->sSmtpPassw = $sValue;
    }
    //**********************************
    //             GETS
    //**********************************
