...
class ComponentZip
{
    ...
    private function fix_path_target()
    {
        if (!$this->sPathTarget && $this->sPathSource) {
            $this->sPathTarget = $this->sPathSource;
        }
    }
    ...
    //             SETS
    //==================================
    public function set_path_source($sValue)
    {
        $this->sPathSource = $this->get_fixed_syspath($sValue);
    }
    public function set_path_target($sValue)
    {
        $this->sPathTarget = $this->get_fixed_syspath($sValue);
    }

    public function set_folder($sFolderName)
    {
        $this->arFolders = array();
        if (trim($sFolderName)) {
            $this->arFolders[] = $sFolderName;
        }
    }
    public function add_folder($sFolderName)
    {
        if (trim($sFolderName)) {
            $this->arFolders[] = $sFolderName;
        }
    }
    public function set_files($sFileName)
    {
        $this->arFiles = array();
        if (trim($sFileName)) {
            $this->arFiles[] = $sFileName;
        }
    }
    public function add_files($sFileName)
    {
        if (trim($sFileName)) {
            $this->arFiles[] = $sFileName;
        }
    ...
    public function set_zipname($sFileName)
    {
        $this->arZipNames = array();
        if (trim($sFileName)) {
            $this->arZipNames[] = $sFileName;
        }
    }
    public function add_zipname($sFileName)
    {
        if (trim($sFileName)) {
            $this->arZipNames[] = $sFileName;
        }
    ...
    //             GETS
    //==================================
    private function get_path_source($sFileFolder)
    {
        return $this->sPathSource.DIRECTORY_SEPARATOR.$sFileFolder;
    }
    private function get_path_target($sFileFolder)
    {
        return $this->sPathTarget.DIRECTORY_SEPARATOR.$sFileFolder;
    }
}
