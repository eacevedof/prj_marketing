...
class ComponentS3
{
    ...
    public function __construct($cfg = null)
    {
        ...
        if (defined("DEV_PROD_CONFIG")) {
            $this->credentials = "/etc/s3cfg";
        }
     ...
     */
    public function put($upload_file_tmp, $uploaded_file_name, $bname = "default", $options = array())
    {
    ...
    public function get_link($bname, $relpath, $options = array())
    {
        ...
        if (isset($options["force_access_key"])) {
            $this->s3_access_key = $options["force_access_key"];
        }
        if (isset($options["force_secret_key"])) {
            $this->s3_secret_key = $options["force_secret_key"];
        }
    ...
    public function upload($upload_file_tmp, $s3_destination/*ej: s3://BUCKET/fichero.htm*/, $options = array())
    {
    ...
    public function change_name_s3_lab($upload_file_tmp, $s3_destination/*ej: s3://BUCKET/fichero.htm*/, $options = array())
    {
    ...
    public function upload_s3_lab($upload_file_tmp, $s3_destination/*ej: s3://BUCKET/fichero.htm*/, $options = array())
    {
    ...
    public function copy_s3_lab($upload_file_tmp, $s3_destination/*ej: s3://BUCKET/fichero.htm*/, $options = array())
    {
    ...
    public function delete_s3_lab($s3_destination/*ej: s3://BUCKET/fichero.htm*/, $options = array())
    {
    ...
    public function debug($str, $options = false)
    {
        if (!isset($this->log)) {
            return;
        }
    ...
    public function download($s3uri, $destination)
    {
    ...
    public function sync_gzipped($local_path, $s3path, $s3sync_options, $options)
    {
    ...
    public function sync_gzipped_types($local_path, $s3path, $s3sync_options, $options)
    {
        $work_path = $this->create_temp_path("/tmp/TMPS3_PHP/", date("Ymd"));
        ...
        if (isset($options["png"])) {
            $this->sync_types_gzipped(".png", "--mime-type='image/png;'", $options);
        }
        if (isset($options["ico"])) {
            $this->sync_types_gzipped(".ico", "--mime-type='image/x-icon;'", $options);
        }
        if (isset($options["gif"])) {
            $this->sync_types_gzipped(".gif", "--mime-type='image/gif;'", $options);
        }
        if (isset($options["jpg"])) {
            $this->sync_types_gzipped(".jpg", "--mime-type='image/jpeg;'", $options);
        }
        $this->sync_resto($options);
    ...
    public function sync_htmls_gzipped($mime_type, $options)
    {
    ...
    public function delete_work_path($work_path)
    {
    ...
    public function sync_types_gzipped($extension, $mime_type, $options)
    {
    ...
    public function sync_resto($options)
    {
    ...
    public function borrar_ficheros_ya_publicados($work_files, $path)
    {
    ...
    public function gzip_files($path)
    {
        $cmd = '/usr/bin/find '.$path.' -type f | while read LINE; do  gzip $LINE;  mv $LINE.gz $LINE; done';
        $this->shell($cmd, $output, $retcode);
    }

    public function _move_files($base_path, $work_files, $path)
    {
        $buffer = "";
        foreach ($work_files as $file) {
            $buffer .= $file."\n";
    ...
    public function create_temp_path($dir, $prefix = '', $mode = 0700)
    {
        if (!is_dir($dir)) {
            mkdir($dir, 0777);
        }
    ...
    public function create_file($buffer, $file, $create_path = true)
    {
        if ($create_path) {
            $path = dirname($file);
            if (!is_dir($path)) {
    ...
    public function shell($cmd, &$output, &$retcode)
    {
