...
final class SignatureService extends AppService
{
    ...
    {
        $sPathfile = $this->getEnvValue("APP_ENCDECRYPT") ?? __DIR__.DIRECTORY_SEPARATOR."encdecrypt.json";
        //prd($sPathfile);
    ...
    private function _load_encdec(): void
    {
        ...
        $this->encdec->setSslMethod($config["sslenc_method"] ?? "");
        $this->encdec->setSslKey($config["sslenc_key"] ?? "");
        $this->encdec->setSaltString($config["sslsalt"] ?? "");
    }
    ...
    public function get_token(): string
    {
        ...
        $token = $this->encdec->getSslEncrypted($instring);
        return $token;
    }
    ...
    public function get_password(): string
    {
        ...
        $password = $this->encdec->getPasswordHashed($word);
        return $password;
    }

    private function _get_remote_ip(): string
    {
        return $_SERVER["REMOTE_ADDR"] ?? "127.0.0.1";
    }

    private function _get_user_agent(): string
    {
    ...
    {
        if (!$arpackage) {
            throw new Exception("Signature wrong token submitted");
        }

        if ($arpackage[0] !== $this->domain) {
            throw new Exception("Signature domain {$this->domain} not authorized");
        }

        if ($arpackage[1] !== $this->_get_remote_ip()) {
            throw new Exception("Signature wrong source {$arpackage[0]} in token");
        }

        if ($arpackage[2] !== md5($this->_get_user_agent())) {
            throw new Exception("Signature wrong user agent in token");
        }

        $data = var_export($this->data, 1);
        $md5 = md5($data);
        if ($arpackage[3] !== $md5) {
            throw new Exception("Signature wrong hash submitted");
        }

        if ($arpackage[4] !== date("Ymd")) {
            throw new Exception("Signature wrong token has expired");
        }
    }
    ...
    public function is_valid(?string $token): bool
    {
        ...
        $instring = $this->encdec->getSslDecrypted($token);
