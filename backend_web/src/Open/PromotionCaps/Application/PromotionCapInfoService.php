...
final class PromotionCapInfoService extends AppService implements IEventDispatcher
{
    ...
    private function _throwPromotionCapException(string $message, int $code = ExceptionType::CODE_INTERNAL_SERVER_ERROR): void
    {
        throw new PromotionCapException($message, $code);
    }

    private function _loadBusinessDataByBusinessSlug(): void
    {
        ...
        if (!$this->businessData) {
            $this->_throwPromotionCapException(__("Business account {0} not found!", $businessSlug), ExceptionType::CODE_NOT_FOUND);
        }
    }
    ...
    private function _loadPromotionUiByPromotionId(): void
    {
        ...
        if (!$this->promotionUi) {
            $this->_throwPromotionCapException(__("Missing promotion UI configuration!"), ExceptionType::CODE_FAILED_DEPENDENCY);
        }
    }
    ...
    public function __invoke(): array
    {
        if (!($this->input["businessSlug"] ?? "")) {
            $this->_throwPromotionCapException(__("No business account provided"), ExceptionType::CODE_BAD_REQUEST);
        }

        if (!($this->input["promotionSlug"] ?? "")) {
            $this->_throwPromotionCapException(__("No promotion name provided"), ExceptionType::CODE_BAD_REQUEST);
        }

        $this->_loadBusinessDataByBusinessSlug();
