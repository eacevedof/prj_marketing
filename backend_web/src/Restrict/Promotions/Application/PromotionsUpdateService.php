        ...
        $this->validator
            ...
            ->add_rule("date_raffle", "date_raffle", function ($data) {
                ...
                if ($value < $data["data"]["date_from"])
                    return __("{0} is lower than {1}", __("Date raffle"), __("Date from"));
                if ($value < $data["data"]["date_to"])
                    return __("{0} is lower than {1}", __("Date raffle"), __("Date to"));
                if ($value > $data["data"]["date_execution"])
                    return __("{0} is greater than {1}", __("Date raffle"), __("Date limit"));
            })
            ...
            ->add_rule("date_execution", "date_execution", function ($data) {
                ...
                if (!$this->datecomp->is_valid($value))
                ...
                $dateraffle = $this->datecomp->add_time($data["data"]["date_raffle"],3600);
                if ($dateraffle > $value)
                    return __("Value must be at least 1 hour after Date raffle");
            })
