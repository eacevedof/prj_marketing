...
final class ConsoleMain
{
    ...
    public static function on_error(Throwable $ex): void
    {
        $uuid = uniqid();
        if ($_POST) {
            lgerr($_POST, "console-exception $uuid POST", "error");
        }
        if ($_GET) {
            lgerr($_GET, "console-exception $uuid GET", "error");
        }
        if ($_REQUEST) {
            lgerr($_REQUEST, "console-exception $uuid REQUEST", "error");
        }
        if ($_ENV) {
            lgerr($_ENV, "console-exception $uuid ENV", "error");
        }
